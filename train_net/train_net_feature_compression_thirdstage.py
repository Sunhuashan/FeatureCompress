...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(twostage_train_loader):
        ...
        for i in range(1,len(d)):
            ...
            input_image= Var(d[i])
            ref_image, mse_loss,  bpp_feature, bpp_z, bpp_mv,bpp_mv_z, bpp =net(input_image, ref_image)
            ...
            distortionloss.append(mse_loss)
            rateloss.append(bpp)
        ...
        if (batch_idx % 10) == 0 and cal_cnt >0:
            log = 'Train epoch:{:02} [{:4}/{:4} ({:3.0f}%)]\t loss {:.6f}\t  PSNR {:.6f}\t Bpp {:.6f}\t Bpp_res {:.6f}\t Bpp_res_z {:.6f}\t Bpp_mv {:.6f}\t Bpp_mv_z {:.6f}\t Aux loss {:.6f}\t lr {}\t'.format(epoch, batch_idx , tot_iter, 100. * batch_idx / (tot_iter), sumloss / cal_cnt, psnr, sumbpp/cal_cnt,sumbpp_feature/cal_cnt,sumbpp_z/cal_cnt,sumbpp_mv/cal_cnt,sumbpp_mv_z/cal_cnt, aux_loss.item(),optimizer.param_groups[0]['lr'])
            logger.info(log)
