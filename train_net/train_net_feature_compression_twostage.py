...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(twostage_train_loader):
        ...
        for i in range(1,len(d)):
            ...
            input_image= Var(d[i])
            ref_image, mse_loss,  bpp_feature, bpp_z, bpp_mv, bpp_mv_z, bpp, offset_info = net(input_image, ref_image)
            ref_image=ref_image.detach()

            mse_loss,  bpp_feature, bpp_z, bpp_mv,bpp_mv_z, bpp = torch.mean(mse_loss),  torch.mean(bpp_feature), torch.mean(bpp_z), torch.mean(bpp_mv),torch.mean(bpp_mv_z), torch.mean(bpp)
        ...
        if (batch_idx % 60) == 0 and bat_cnt > 7:
            log = 'Train epoch:{:02} [{:4}/{:4} ({:3.0f}%)]\t loss {:.6f}\t  PSNR {:.6f}\t Bpp {:.6f}\t Bpp_res {:.6f}\t Bpp_res_z {:.6f}\t Bpp_mv {:.6f}\t Bpp_mv_z {:.6f}\t Aux loss {:.6f}\t lr {}\t'.format(epoch, batch_idx , tot_iter*6, 100. * batch_idx / (tot_iter*6), sumloss / cal_cnt, psnr, sumbpp/cal_cnt,sumbpp_feature/cal_cnt,sumbpp_z/cal_cnt,sumbpp_mv/cal_cnt,sumbpp_mv_z/cal_cnt, aux_loss.item(),optimizer.param_groups[0]['lr'])
            logger.info(log)
...
if __name__ == "__main__":
    ...
    save_model(model, global_step, train_lambda, stage)
