...
from plt.hevc_draw_points import hevcddrawplt
torch.backends.cudnn.enabled = True

from fvc_net.RDloss import *
from dataset import HEVC_ClassDDataSet
import os
...
parser.add_argument('--testhevcD', action='store_true')
parser.add_argument('--testhevcE', action='store_true')
...
def testhevcD(global_step, testfull=False):
    with torch.no_grad():
        ...
        sumpsnr /= cnt
        summsssim /= cnt
        log = "HEVC_ClassD dataset : average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf , average bpprate:%.6lf\n" % (sumbpp, sumpsnr, summsssim, sumbpprate)
        logger.info(log)
        sumbpp = sumbpp.cpu().numpy()
        sumpsnr = sumpsnr.cpu().numpy()
        summsssim = summsssim.cpu().numpy()
        hevcddrawplt([sumbpp], [sumpsnr], [summsssim], global_step, testfull=testfull)
...
if __name__ == "__main__":
    ...
    if args.testhevcD:
        ...
        testhevcD(global_step, testfull=True)
        exit(0)
