...
from draw_points import hevcedrawplt
torch.backends.cudnn.enabled = True

from fvc_net.RDloss import *
from torchvision import transforms
from fvc_net.video import VideoFolder
from dataset import HEVC_ClassFDataSet
...
parser.add_argument('--testhevcF', action='store_true')
...
def testhevcF(global_step, testfull=False):
    with torch.no_grad():
        ...
        sumpsnr /= cnt
        summsssim /= cnt
        log = "HEVC_ClassF dataset : average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf\n" % (sumbpp, sumpsnr, summsssim)
        logger.info(log)
...
if __name__ == "__main__":
    ...
    if args.testhevcF:
        ...
        test_dataset = HEVC_ClassFDataSet(refdir=ref_i_dir, testfull=True)
        print('testing HEVC_ClassF')
        testhevcF(global_step, testfull=True)
        exit(0)
