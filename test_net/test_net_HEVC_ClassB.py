...
from plt.hevc_draw_points import hevcbdrawplt
torch.backends.cudnn.enabled = True
from fvc_net.RDloss import *
from dataset import HEVC_ClassBDataSet
import os
...
def parse_config(config):
    ...
    if 'train_lambda' in config:
        ...
        # ref_i_dir = gei(train_lambda)
        # ref_i_dir = get_chen2020anchor_i(train_lambda)
        #ref_i_dir = get_bmshj2018_hyperprior_i(train_lambda)
        ref_i_dir = get_mbt2018_i(train_lambda)


def testhevcB(global_step, testfull=False):
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ...
            for i in range(seqlen):
                ...
                # reconframe, mse_loss, bpp_feature, bpp_z, bpp_mv, bpp = net(ref_image, input_image
        ...
        sumpsnr /= cnt
        summsssim /= cnt
        log = "HEVC_ClassB dataset : average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf , average bpprate:%.6lf\n" % (sumbpp,sumpsnr, summsssim, sumbpprate)
        logger.info(log)
        sumbpp=sumbpp.cpu().numpy()
        sumbpprate = sumbpprate.cpu().numpy()
        sumpsnr=sumpsnr.cpu().numpy()
        summsssim=summsssim.cpu().numpy()
        hevcbdrawplt([sumbpp], [sumpsnr], [summsssim], global_step, testfull=testfull)
