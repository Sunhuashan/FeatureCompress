...
from plt.mcl_cjv_draw_points import mcldrawplt
torch.backends.cudnn.enabled = True

from fvc_net.RDloss import *
from dataset import MCL_JCVDataSet
...
def testmcl(global_step, testfull=False):
    with torch.no_grad():
        ...
        sumbpprate /= (cnt * 1024)
        summsssim /= cnt
        log = "MCL_JCV dataset :  average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf , average bpprate:%.6lf\n" % (sumbpp, sumpsnr, summsssim, sumbpprate)
        logger.info(log)
        sumbpp = sumbpp.cpu().numpy()
        sumpsnr = sumpsnr.cpu().numpy()
        summsssim = summsssim.cpu().numpy()
        mcldrawplt([sumbpp], [sumpsnr], [summsssim], global_step, testfull=testfull)
...
if __name__ == "__main__":
    ...
    if args.testmcl:
        ...
        testmcl(global_step, testfull=True)
        exit(0)
